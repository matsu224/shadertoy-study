...
)
{
    ...
    float mask = 1.0 - smoothstep(0.7, 0.9, d);
    ...
    p = rot * p;
    float vx = sin(p.x * 20.0);
    vx = (vx + 1.0) / 2.0;

    vec3 colorA = vec3(0.1, 0.2, 0.8);
    vec3 colorB = vec3(1.0, 0.3, 0.1);
    vec3 colorC = vec3(1.0, 1.0, 0.0);
    vec3 stripeColor = mix(colorA, colorB, vx);

    vec3 result = mix(stripeColor, colorC, mask);
